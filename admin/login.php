...
if (!empty($_POST['hash_one']) && !empty($_POST['hash_sec'])) {
  ...
  if (!empty($_POST['ga_playertest'])) {
    $sql = "SELECT `username`,`ga_token` FROM `ga_players` WHERE `username`='".prot($_POST['hash_one'])."' AND `passwd`='".md5($_POST['hash_sec'])."' LIMIT 1";
  } else {
    $sql = "SELECT `username`,`ga_token` FROM `admins` WHERE `username`='".prot($_POST['hash_one'])."' AND `passwd`='".md5($_POST['hash_sec'])."' LIMIT 1";
  }

  $this_admin=mysql_fetch_array(mysql_query($sql));
